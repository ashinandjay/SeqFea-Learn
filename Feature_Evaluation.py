...
filename = sys.argv[1]
#filename='RNA_Psednc_PSTNP_TNC_DNC_MonoKGap.csv'
feature = pd.read_csv(filename,header='infer',index_col=0,float_precision='round_trip')
...
X_=scale(X_)
y_= np.array(label.values.ravel())
...
for train, test in skf.split(X,y):
    X_=(X)
    clf_svm= clf_svm.fit(X_[train], y[train]) #train model
    ac_svm.append(accuracy_score(y[test],clf_svm.predict(X_[test]))) # add to accuracy list
    mean_fpr = np.linspace(0, 1, 100)
    probas_ = clf_svm.fit(X[train], y[train]).predict_proba(X_[test])
...
methods = ['SVM', 'KNN', 'RF', 'LightGBM','XGBoost','AdaBoost','Bagging','Extra-Trees', 'GNB','GBDT','DNN','CNN','RNN']
...
plt.plot(mean_fpr, mean_tpr, color='darkgoldenrod',
         label=r'LightGBM (AUC = %0.4f)' % (mean_auc),
         lw=2, alpha=.8)
...
plt.plot(mean_fpr, mean_tpr, color='green',
         label=r'XGBoost (AUC = %0.4f)' % (mean_auc),
         lw=2, alpha=.8)
...
plt.plot(mean_fpr, mean_tpr, color='slategrey',
         label=r'AdaBoost (AUC = %0.4f)' % (mean_auc),
         lw=2, alpha=.8)
...
plt.plot(mean_fpr, mean_tpr, color='c',
         label=r'Bagging (AUC = %0.4f)' % (mean_auc),
         lw=2, alpha=.8)
...
plt.plot(mean_fpr, mean_tpr, color='blue',
         label=r'Extra-Trees (AUC = %0.4f)' % (mean_auc),
         lw=2, alpha=.8)
...
plt.plot(mean_fpr, mean_tpr, color='deeppink',
         label=r'GBDT (AUC = %0.4f)' % (mean_auc),
         lw=2, alpha=.8)
...
plt.plot(mean_fpr, mean_tpr, color='greenyellow',
         label=r'RNN (AUC = %0.4f)' % (mean_auc),
         lw=2, alpha=.8)
