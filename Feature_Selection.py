...
feature = pd.read_csv(featurefile,header=None, index_col=0,float_precision='round_trip')
...
def L1SVM_selection(X,y,parameter=1):
    lsvc = LinearSVC(penalty="l1", dual=False,C=parameter)
    model=SelectFromModel(lsvc)
    lsvcresult = model.fit(X,y.values.ravel()).get_support(indices=True)
    lsvcresult = X[X.columns[lsvcresult]]
    lsvcresult.to_csv("L1SVM_out.csv")
    return None

# Feature selection L1-LR
def L1LR_selection(X,y,parameter=1):
    llr=LogisticRegression(penalty="l1", C=parameter,max_iter=500)
    model=SelectFromModel(llr)
    llrresult = model.fit(X,y.values.ravel()).get_support(indices=True)
    llrresult = X[X.columns[llrresult]]
    llrresult.to_csv("L1LR_out.csv")
    return None
...
# Feature selection Lasso
Lasso_selection(feature,label,alpha=0.01)
# Feature selection Elastic-Net
Elastic_selection(feature,label,alpha =0.1,l1_ratio=0.1)
# Feature selection L1-SVM
L1SVM_selection(feature,label,parameter=1)
# Feature selection L1-LR
L1LR_selection(feature,label,parameter=1)
# Feature selection CHI2
Chi2_selection(selected_number,feature,label)
# Feature selection Pearson Correlation
cor_feature = cor_selector(selected_number,feature, label.values.ravel())
PCresult = feature[cor_feature]
PCresult.to_csv("PearsonCorrelation_out.csv")
# Feature selection ExtraTree   
ExtraTree_selection(selected_number,feature,label)
# Feature selection XGBoost
XGB_selection(selected_number,feature,label)
# Feature selection SVM-RFE 
SVM_RFE_selection(selected_number,feature,label)
# Feature selection LOG-RFE
LOG_RFE_selection(selected_number,feature,label)
# Feature selection Mutual Info
Mutual_Info_selection(selected_number,feature,label)
 # Feature selection MRMR
X=np.array(feature)
y=np.asarray(label)
y = y[:, 0]
MRMRresult=mrmr(X,y,n_selected_features=selected_number)
MRMRresult = feature[feature.columns[MRMRresult]]
...
JMIresult = feature[feature.columns[JMIresult]]
...
traceresult = feature[feature.columns[idx]]
...
specresult = feature[feature.columns[idx[0:selected_number]]]
...
fisherresult = feature[feature.columns[idx[0:selected_number]]]
...
tscoreresult = feature[feature.columns[idx[0:selected_number]]]
...
#elif(option == 'all'):
#	# Run All methods
#    Lasso_selection(selected_number,feature,label)
#    Elastic_selection(selected_number,feature,label)
#    L1SVM_selection(selected_number,feature,label)
#    Chi2_selection(selected_number,feature,label)
#    cor_feature = cor_selector(selected_number,feature,label.values.ravel())
#    PCresult = feature[cor_feature]
#    PCresult.to_csv("PearsonCorrelation_out.csv")
